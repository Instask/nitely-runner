...
run. Configs may set `runnerStatePath` to persist the runner-local assignment
projection after every lifecycle transition for restart recovery and
troubleshooting. Executors can also return artifact metadata as
`evidenceArtifacts`; the runner reports it through metadata-only
